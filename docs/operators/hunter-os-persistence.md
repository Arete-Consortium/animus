...
Next: typed Hunter payload validation, the read repository and an idempotent four-record import. The generic envelope contract added here does not validate exact Hunting Horn songs, ammo tables, sources, or publish eligibility. Those facts must remain unavailable for authoritative chat until the domain layer and audits are implemented. Live database inspection, legacy-layout conversion if needed, bot ownership and end-to-end shared-chat isolation remain deployment gates.
