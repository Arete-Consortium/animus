...
Next is the import preview: map the four existing YAML candidates without dropping information, preserve source revision/path/section/hash and verification labels, produce held/conflict counts, then prove repeated imports are no-ops. The expanded 101 text blocks remain separate staging work. Relationships are validated as IDs here; target existence and cross-record consistency belong to that import/repository integration work. Existing PR #129's YAML loader and chat modules must be adapted deliberately to this API, not merged wholesale.
